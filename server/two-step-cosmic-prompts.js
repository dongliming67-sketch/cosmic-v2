...
- **功能过程生成规则**：
    ...
    - **规则 7.1【极重要：业务对象具体化原则 - 拒绝笼统描述】**：
      ...
      **严禁使用的笼统描述（反面典型，绝对不能出现！）**：
      - ❌ 新增任务（太模糊！什么任务？新增的是配置还是执行计划？）
      - ❌ 查询信息（太模糊！查询的是详情还是列表？）
      - ❌ 导出数据（太模糊！导出的是Excel报表还是CSV原始流？）
      - ❌ 删除规则（太模糊！删除的是全局规则还是单次任务规则？）
      - ❌ 查询结果（极其笼统！查询什么结果？必须写明业务场景！）
      - ❌ 数据处理（极其笼统！处理什么数据？怎么处理？）
      - ❌ 信息展示（极其笼统！展示什么信息？）
      - ❌ 结果返回（极其笼统！返回什么结果？）
      
      **功能过程最小长度检查**：
      - 功能过程名称必须至少6个字以上
      - 如果名称少于6个字，说明描述过于笼统，必须补充业务场景
      - 示例：查询结果(4字) → 低空保障任务执行结果查询(12字)
...
- **【新增-严禁连接符】**：绝对禁止在数据组名称中使用连接符"-"来拼接动作或描述！
  - ❌ 错误示例：低空保障任务运行日志表-读取低空保障任务
  - ❌ 错误示例：参数配置数据表-查询数据
  - ✅ 正确做法：低空保障任务查询日志表（将动作融入名称）
  - ✅ 正确做法：低空保障任务数据导出表（使用场景化后缀）

**数据组正确去重方式**：
- 使用不同的业务场景后缀：xxx查询表、xxx导出表、xxx配置表
- 使用不同的数据类型后缀：xxx请求参数、xxx响应数据、xxx操作日志
- 使用不同的操作类型：xxx创建请求、xxx查询请求、xxx删除请求
