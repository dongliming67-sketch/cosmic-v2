...
function getActiveClientConfig(userConfig = null) {
  ...
  if (userConfig && userConfig.provider === 'iflow' && !userConfig.apiKey) {
    ...
    const iflowModel = userConfig.model || process.env.IFLOW_MODEL || 'deepseek-v3';
  ...
  // auto 模式：优先使用 心流DeepSeek-V3（推荐），其次 Gemini，其次智谱（国内稳定）
  if (provider === 'auto') {
    // 最优先：心流开放平台 DeepSeek-V3（速度快，无并发限制）
    if (process.env.IFLOW_API_KEY) {
      const iflowBaseUrl = process.env.IFLOW_BASE_URL || 'https://apis.iflow.cn/v1';
      const iflowModel = process.env.IFLOW_MODEL || 'deepseek-v3';
      console.log(`[Auto] 使用心流开放平台 DeepSeek-V3`);
      return {
        client: new OpenAI({
