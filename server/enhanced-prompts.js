...
- 必须简洁，不超过15个字
- 格式：接收XXX请求数据
- ❌ 禁止：接收中兴小区级智算板指标-日解析数据和小区工参数据，生成中兴小区级智算板指标-日数据
- ✅ 正确：接收中兴小区级智算板指标日数据

**示例**：
- 子过程描述：接收飞行任务创建请求
- 数据组：飞行任务创建请求
- 数据属性：请求标识、任务名称、航线编号、无人机编号、计划时间
...
- 必须简洁，不超过15个字
- 格式：读取XXX相关数据
- ❌ 禁止：读取中兴小区级智算板基础指标计算相关数据
- ✅ 正确：读取中兴小区级智算板基础指标基础指标数据

**示例**：
- 子过程描述：读取航线和无人机数据
- 数据组：航线与无人机资源
- 数据属性：航线编号、航线名称、无人机编号、无人机状态、电量
...
- 必须简洁，不超过15个字
- 格式：记录XXX处理结果
- ❌ 禁止：记录中兴小区级智算板基础指标计算操作日志
- ✅ 正确：记录指标计算日志

**示例**：
- 子过程描述：保存飞行任务配置
- 数据组：飞行任务配置表
- 数据属性：任务编号、任务名称、任务状态、航线编号、创建时间
